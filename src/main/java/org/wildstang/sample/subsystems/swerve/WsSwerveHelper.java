...
public class WsSwerveHelper {
    ...
    /**
     * sets the robot in the mobile defensive "crab" mode, where all modules are
     * aligned
     * 
    ...
    public SwerveSignal setCrab(double i_tx, double i_ty, double i_gyro) {
        ...
        swerveSignal = new SwerveSignal(new double[] { magnitude, magnitude, magnitude, magnitude },
                new double[] { direction, direction, direction, direction });
    ...
    public SwerveSignal setDrive(double i_tx, double i_ty, double i_rot, double i_gyro) {
        ...
        xCoords = new double[] { xTrans + rotMag * Math.cos(baseV), xTrans + rotMag * Math.sin(baseV),
                xTrans - rotMag * Math.sin(baseV), xTrans - rotMag * Math.cos(baseV) };
        yCoords = new double[] { yTrans + rotMag * Math.sin(baseV), yTrans - rotMag * Math.cos(baseV),
                yTrans + rotMag * Math.cos(baseV), yTrans - rotMag * Math.sin(baseV) };

        // create drivesignal, with magnitudes and directions of x and y
        swerveSignal = new SwerveSignal(
                new double[] { getMagnitude(xCoords[0], yCoords[0]), getMagnitude(xCoords[1], yCoords[1]),
                        getMagnitude(xCoords[2], yCoords[2]), getMagnitude(xCoords[3], yCoords[3]) },
                new double[] { getDirection(xCoords[0], yCoords[0]), getDirection(xCoords[1], yCoords[1]),
                        getDirection(xCoords[2], yCoords[2]), getDirection(xCoords[3], yCoords[3]) });
        swerveSignal.normalize();
     ...
     * @param i_heading direction of translational vector, in field centric bearing
     *                  degrees
     * @param i_rot     the rotational joystick value, created by the heading
     *                  controller
     * @param i_gyro    the gyro value, field centric, in bearing degrees
     * @return SwerveSignal that is the command for the robot to move
     */
    public SwerveSignal setAuto(double i_power, double i_heading, double i_rot, double i_gyro, double xOffset,
            double yOffset) {
        return setDrive(i_power * -Math.sin(Math.toRadians(i_heading)) + xOffset * .5,
                i_power * -Math.cos(Math.toRadians(i_heading)) + yOffset * .5, i_rot, i_gyro); // TODO: change xxoffset
                                                                                               // multiplier
    }

    /**
     * automatically creates a rotational joystick value to rotate the robot towards
     * a specific target
     * 
     * @param i_target target direction for the robot to face, field centric,
     *                 bearing degrees
     * @param i_gyro   the gyro value, field centric, in bearing degrees
    ...
    public double getAutoPower(double pathVel) {
        if (pathVel == 0)
            return 0;
        double guess = pathVel * DriveConstants.DRIVE_F_V + DriveConstants.DRIVE_F_K
                + 0.02 * (pathVel - prevVel) * DriveConstants.DRIVE_F_I;
